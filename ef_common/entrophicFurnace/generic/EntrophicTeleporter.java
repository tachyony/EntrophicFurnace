...
public class EntrophicTeleporter extends Teleporter
{
    ...
    {
        if (this.worldServerInstance.provider.dimensionId == 143)
        {
            par1Entity.setLocationAndAngles(0, 64, 0, par1Entity.rotationYaw, 0.0F);
            par1Entity.motionX = par1Entity.motionY = par1Entity.motionZ = 0.0D;
            /*if (!this.placeInExistingPortal(par1Entity, par2, par4, par6, par8))
            {
                this.makePortal(par1Entity);
                this.placeInExistingPortal(par1Entity, par2, par4, par6, par8);
                par1Entity.setLocationAndAngles(0, 70, 0, par1Entity.rotationYaw, 0.0F);
                par1Entity.motionX = par1Entity.motionY = par1Entity.motionZ = 0.0D;
            }*/
        }
        else
        {
            NBTTagCompound tag = par1Entity.getEntityData();
            NBTBase xTag = tag.getTag("oldx");
            NBTBase yTag = tag.getTag("oldy");
            NBTBase zTag = tag.getTag("oldz");
            double x = 0;
            double y = 64;
            double z = 0;
            if (xTag != null) {
                x = ((NBTTagDouble)xTag).data;
            }
            
            if (yTag != null) {
                y = ((NBTTagDouble)yTag).data + 1;
            }
            
            if (zTag != null) {
                z = ((NBTTagDouble)zTag).data;
            }
    ...
    public boolean placeInExistingPortal(Entity par1Entity, double par2, double par4, double par6, float par8)
    {
        ...
        if (d3 >= 0.0D)
        {
            if (flag)
            {
                this.destinationCoordinateCache.add(j1, new EntrophicPortalPosition(this, i, j, k, this.worldServerInstance.getTotalWorldTime()));
        ...
        for (k2 = 0; k2 < 4; ++k2)
        {
            for (i3 = 0; i3 < 4; ++i3)
            {
                for (l2 = -1; l2 < 4; ++l2)
                {
                    ...
                    /** change these blocks **/
                    this.worldServerInstance.setBlock(k3, j3, i4, flag ? Block.cobblestoneMossy.blockID : Block.portal.blockID, 0, 2);
                }
            }
            for (i3 = 0; i3 < 4; ++i3)
            {
